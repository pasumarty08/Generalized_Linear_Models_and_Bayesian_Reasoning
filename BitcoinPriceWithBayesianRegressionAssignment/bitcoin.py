...
def computeDelta(wt, X, Xi):
    ...
    similarList = []
    lengthX = len(X)-1
    meanX = np.mean(X[:lengthX])
    i = 0;
    while(i < 50):
    	meanXi = np.mean(Xi.iloc[i][:lengthX])
    	numeratorEQ6=0.00
        j = 0
    	while (j<lengthX):
    		numeratorEQ6+= (X.iloc[j]-meanX)*(Xi.iloc[i][j]-meanXi)
	        j = j+1
    	denominatorEQ6 = lengthX * np.std(X[:lengthX]) * np.std(Xi.iloc[i][:lengthX])
    	similiarity = float(numeratorEQ6/denominatorEQ6)
    	similarList.append(similiarity)
        i = i+1
    	
    expNum = 0
    expDen = 0 
    for i in range(0,50):
    	expNum += Xi.iloc[i][lengthX]*(np.exp(wt*(similarList[i])))
    	expDen += (np.exp(wt*(similarList[i])))
    result = float(expNum/expDen)
    return result
